...
<head>
    ...
    <title>Hasil Rekomendasi Peminatan</title>
</head>
<body>
    ...
    <div class="col-lg-15 grid-margin stretch-card">
        ...
        <tbody>
            @foreach ($decisionMatrix as $subalternatifId => $kriteriaValues)
                <tr>
                    <td>{{ $alternatifNames[$subalternatifId] ?? 'Alternatif Tidak Ditemukan' }}</td>
        ...
        <tbody>
            @foreach ($positiveDistances as $subalternatifId => $positiveDistance)
                <tr>
                    ...
                    <td>{{ number_format($negativeDistances[$subalternatifId], 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @foreach ($preferenceScores as $subalternatifId => $score)
                <tr>
                    <td>{{ $alternatifNames[$subalternatifId] ?? 'Alternatif Tidak Ditemukan' }}</td>
    ...
    <div class="col-lg-15 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive">
                <p>
    Alternatif terbaik berdasarkan perhitungan adalah: 

    @foreach($topAlternatives as $index => $alternative)
        <strong>{{ $alternative['nama'] }}</strong>@if (!$loop->last),@endif
    @endforeach
</p>

    <a href="{{ route('penelusuran-minat.index') }}" class="btn btn-inverse-secondary btn-fw btn-sm">Kembali</a>
</div>
</div>
</div>
</div>
